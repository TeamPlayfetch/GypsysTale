...
////// Emissive:
                ...
                float3 finalColor = emissive + (((node_544*floor(max(0,dot(lightDirection,normalDirection)) * _BandValue) / (_BandValue - 1))+(floor(node_3269 * _BandValue) / (_BandValue - 1)*floor(pow(node_3269,(exp2(lerp(1,11,_Gloss))*20.0)) * _BandValue) / (_BandValue - 1)))*_LightColor0.rgb*attenuation);
